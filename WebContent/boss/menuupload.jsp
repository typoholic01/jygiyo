...
if(isMultipart){	// Multipart로 전송되었는지
	...
	while(it.hasNext()){
		...
		if(item.isFormField()){		// id, title, content 파일이 아닌 text형태면 이 조건으로 들어 온다
			if(item.getFieldName().equals("food_category")){
				food_category = item.getString("utf-8");	
			}else if(item.getFieldName().equals("food_name")){
				food_name = item.getString("utf-8");
			}else if(item.getFieldName().equals("food_price")){
				food_price = item.getString("utf-8");
			}else if(item.getFieldName().equals("food_size")){
				food_size = item.getString("utf-8");
			}else if(item.getFieldName().equals("seq_store")){
				seq_store = item.getString("utf-8");
			}		
...
int seq = Integer.parseInt(seq_store);
int price = Integer.parseInt(food_price);
boolean isS = d.foodCtrl.insertFood(new FoodDto(seq, food_category, food_name, price, food_size, filename, "0"));
if(isS){
	%>
